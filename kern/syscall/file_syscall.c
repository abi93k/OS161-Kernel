...
int open(char *filepath, int flags, mode_t mode)
{
	...
	*/
	...
	int index=3;
	while(curthread->t_fdtable[index++]!=0)
	{}
	index--;	
...
int close(int fd)
{
	/*	||TODO||
		...
		1. ERROR - Check if the fd is valid
		2. ERROR - Check if there is a entry in the fdtable for that fd
		3. Decrement the reference count and free memory if zero.
		4. USE vfs_close()
	*/
...
int chdir(const char *newpath)
{
	/*	||TODO||
		...
		1. ERROR - Check if the newpath is a invalid pointer
		2. ERROR - Check for errors about the path
		3. Use copyinstr() to copy the filepath to kernel memory.
		4. USE vfs_chdir()
	*/
...
void copyvalues(int oldfd,int newfd)
{
...
int dup2(int oldfd,int newfd)
{
	/*	||TODO||
		...
		1. ERROR - Check if newfd and old are valid
		2. ERROR - Check if filesystem is full.
		3. Check if both newfd and oldfd are the same.
		4. Close the newfd using sys_close
		5. Aquire the lock on oldfd
		6. Copy values from oldfd to new fd
		7. Release the lock and return newfd
	*/
